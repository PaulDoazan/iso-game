...
export class TopDownScene extends Container {
  ...
  constructor(screenWidth: number, screenHeight: number) {
    ...
    const matrix = Array(this.gridSize).fill(null).map(() => Array(this.gridSize).fill(0))
  ...
  private createGrid() {
    ...
    // We'll create a larger grid to ensure it covers the entire screen when panned
    const extendedGridSize = Math.max(this.gridSize, Math.ceil(this.screenWidth / this.tileSize) + 10)
    
    for (let x = 0; x < extendedGridSize; x++) {
      for (let y = 0; y < extendedGridSize; y++) {
  ...
  handleClick(screenX: number, screenY: number) {
    ...
    const clampedGridX = Math.max(0, Math.min(this.gridSize - 1, gridX))
    const clampedGridY = Math.max(0, Math.min(this.gridSize - 1, gridY))
    
    // Get current sphere position in grid coordinates
    const currentPos = this.sphere.getPosition()
    const startGridX = Math.max(0, Math.min(this.gridSize - 1, Math.floor(currentPos.x / this.tileSize)))
    const startGridY = Math.max(0, Math.min(this.gridSize - 1, Math.floor(currentPos.y / this.tileSize)))
    
    // Calculate path (even though no obstacles, pathfinding will give us a straight path)
    this.grid = new PF.Grid(Array(this.gridSize).fill(null).map(() => Array(this.gridSize).fill(0)))
